        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-2 rounded-lg px-3.5 py-2 text-xs font-medium transition-colors hover:bg-amber-400/[0.06] disabled:opacity-50"
              style={{ border: "1px solid rgba(245,158,11,0.28)", color: "#f59e0b" }}
            >
              <svg
          ...
          >
            ...
            >
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-600">Active positions across asset classes</p>
              </div>
              <span
            ...
            {assets.length > 0 && (
              <div
                className="grid min-w-[580px] px-5 py-2.5 text-[9px] font-bold uppercase tracking-[0.13em] text-slate-700"
                style={{
              ...
              <div className="overflow-x-auto">
                {assets.map((asset, idx) => {
                  ...
                  return (
                    <div
                      key={asset.id}
                      className="group grid min-w-[580px] items-center px-5 transition-colors hover:bg-white/[0.017]"
                      style={{
                        gridTemplateColumns: "minmax(0,1fr) 110px 72px 88px minmax(160px,200px)",
                        paddingTop: "11px",
                        paddingBottom: "11px",
                    ...
                    >
                          ...
                          {asset.holding ? (
                            <p className="mt-0.5 text-[10px] text-slate-500">
                              <span className="font-semibold text-amber-400/80">
                                {fmtQty(asset.holding.quantity)} {asset.holding.symbol}
                              </span>
                              <span className="mx-1.5 text-slate-700">·</span>
                              <span>Avg {inr(asset.holding.avg_buy_price)}</span>
                            </p>
                          ) : (
                            <p className="font-mono text-[9px] text-slate-700">{asset.id.slice(0, 10)}…</p>
                          )}
                        </div>
                      </div>
                      ...
                      {/* Type badge */}
                      <div>
                        <span className={`inline-block rounded border px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${cfg.badge}`}>
                          {asset.asset_type.replace("_", " ")}
                        </span>
                      </div>

                      {/* Category */}
                      <span className="text-[11px] capitalize text-slate-600">{asset.category}</span>

                      {/* Liquidity */}
                      <div className="flex items-center gap-1.5">
                        <span className={`h-1.5 w-1.5 rounded-full ${asset.liquidity_tier === "LIQUID" ? "bg-emerald-400" : "bg-orange-400"}`} />
                        <span className={`text-[11px] ${asset.liquidity_tier === "LIQUID" ? "text-emerald-400" : "text-orange-400"}`}>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.14)", color: "#f87171" }}
                        >
                          Remove
                        </button>
                      </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: `linear-gradient(90deg, ${a.bar} 0%, transparent 70%)` }}
      />
